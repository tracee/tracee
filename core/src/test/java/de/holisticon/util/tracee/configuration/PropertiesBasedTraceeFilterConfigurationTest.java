...
import static de.holisticon.util.tracee.configuration.PropertiesBasedTraceeFilterConfiguration.CONFIGURATION_KEY_PREFIX;
import static de.holisticon.util.tracee.configuration.PropertiesBasedTraceeFilterConfiguration.GENERATE_REQUEST_ID;
...
public class PropertiesBasedTraceeFilterConfigurationTest {
	...
	public void testGeneratedRequestIdLength() {
		when(propertyChain.getProperty(GENERATE_REQUEST_ID)).thenReturn("1");
		assertThat(unit.generatedRequestIdLength(), equalTo(1));
	}

	@Test
	public void testGeneratedRequestIdNonNumericMeansZero() {
		when(propertyChain.getProperty(GENERATE_REQUEST_ID)).thenReturn("false");
		assertThat(unit.generatedRequestIdLength(), equalTo(0));
	}

	@Test
	public void testGeneratedSessionIdLength() {
		when(propertyChain.getProperty((PropertiesBasedTraceeFilterConfiguration.GENERATE_SESSION_ID))).thenReturn("42");
